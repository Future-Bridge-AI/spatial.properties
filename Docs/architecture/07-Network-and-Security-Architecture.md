...
## 1. New Controls for Agents

- **Device Identity & mTLS:** x.509 device certs per agent, rotated ≤ 30 days; mTLS to presigner and Edge Sync.  
- **Signed URL Claims (Extended):** `device_id`, `attestation_nonce`; reject reuse across devices.  
- **Attestation (Optional):** EAT/Android/Apple attestation verified server‑side; bind to nonce.

## 2. P2P/DTN Security

- **Replay Protection:** signed exchanges with nonces; strict hash verification before commit.  
- **Quarantine:** peer reputation and deny lists; auto‑rotation of presigned URLs for affected cohorts.  
- **Rate Limits:** per‑peer/token; DTN queue backpressure signals.

## 3. Beacon Security

- Short TTL pointers; signed beacon updates; owner keys per venue; disable regionally on anomaly.

## 4. Data Plane (Clarification)

- CDN remains the **primary** delivery; P2P/DTN and beacons are **supplemental** paths feeding CSP‑1 tiles and hints.